...
struct Polygon * polyWindowToViewport(Region window, Region viewport, struct Polygon *oldPoly)
{
    struct Polygon *newPolyHead = NULL, *newPolyTail = NULL, *nextNode = oldPoly;
    ...
    applyTranslation(&m, -1*window.windowLeft, -1*window.windowBottom);
    do {
        initHomoVector(&initial, nextNode->P.x, nextNode ->P.y);
        matrixVectorProduct(&final, m, initial);
        twoDCoord(&point, final);
        addPtNodeToList(&newPolyHead, &newPolyTail, point);
        nextNode = nextNode->next;
       } while(nextNode != oldPoly);
    return newPolyHead;
}
