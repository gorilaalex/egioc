...
isPtInTriangle(PlanePoint pt, PlanePoint ptTr1, PlanePoint ptTr2, PlanePoint ptTr3) {
  ...
  DBG("'isPtInTriangle' : pt{%d %d} with a = %lf, b = %lf\n", pt.x, pt.y, a, b);
...
computeZBuffer(XPM *canvas, ZBuffer *zb, SpaceViewSettings space, SpaceObjData data, Region window, Region viewport) {
  ...
  for(id = 0; id < data.trigs.length; ++id) {
    ...
    DBG("'computeZBuffer' : {xmin, ymin} = {%3d, %3d}, {xmax, ymax} = {%3d, %3d}\n", xmin, ymin, xmax, ymax);
...
projectZBuffer(XPM *canvas, ZBuffer *zb, unsigned short clrBase, unsigned short clrShadeCnt) {
  ...
  double absMaxz = fabs(zb->minz);
  int clrIncrement = (int)((absMinz + absMaxz)/clrShadeCnt);
  double absZData;
  int xpix, ypix;
  int clrOffset;

  DBG("'projectZBuffer' : zb->minz = %lf, zb->maxz = %lf, clrIncrement = %d\n", absMinz, absMaxz, clrIncrement);
  getchar();
